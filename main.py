...
def get_network_metrics():
    ...
    return dict(zip(("byte_out", "byte_in", "pack_out",
                     "pack_in", "err_in", "err_out",
                     "drop_in", "drop_out"), metrics))
...
def update_df():
    ...
    metrics = [
        list(get_speed_metrics().values())[-2:],
    ...
    # save dataframe, append data to metrics.csv
...
def generate_plots():
    ...
    # distribute x-axis labels to 6 per plot
    x_tick_dist = math.floor(len(df["Datetime"]) / 6)
    plt.xticks(df["Datetime"][0::x_tick_dist])
    plt.show()
